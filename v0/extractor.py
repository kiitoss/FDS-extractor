...
IDENTIFIERS = {"name": "Nom du produit", "code": "Code du produit", "ufi": "UFI"}
...
def get_product_data(pdf_path, pages=[0]):
    ...
    return {"pdf": pdf_path, "version": version, "n_pages": n_pages, "data": data}
...
def extract_images(pdf_path, output_path, min_width, min_height):
    ...
    if images:
        for i, img in enumerate(images, start=1):
            ...
            with PIL.Image.open(io.BytesIO(data.get("image"))) as image:
                output_filename = (
                    f"{output_path}/{"skiped/" if skiped else ""}image_{i+1}"
                )
    ...
    if drawings:
        for i, drawing in enumerate(drawings):
            ...
            output_filename = (
                f"{output_path}/{"skiped/" if skiped else ""}drawing_{i+1}.png"
            )
...
def load_folder_images(folder_path):
    ...
    for image in os.listdir(folder_path):
        ...
        if not os.path.isfile(image_path):
            continue
        images.append(
            {"name": image, "image": cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)}
        )
    return images
